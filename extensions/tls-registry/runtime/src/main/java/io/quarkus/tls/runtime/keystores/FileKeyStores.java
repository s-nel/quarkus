...
public class FileKeyStores {
    private FileKeyStores() {
        // Avoid direct instantiation
    }

    public static KeyStoreAndKeyCertOptions verifyKeyStoreFile(KeyStoreConfig config, KeyStoreFileConfig fileConfig,
            Vertx vertx, String name,
            String type) {
        var options = toOptions(fileConfig, config.credentialsProvider(), name, type);
    ...
    public static TrustStoreAndTrustOptions verifyTrustStoreFile(TrustStoreConfig config, TrustStoreFileConfig fileConfig,
            Vertx vertx, String name,
            String type) {
        KeyStoreOptions options = toOptions(fileConfig, config.credentialsProvider(), name, type);
        KeyStore ks = loadKeyStore(vertx, name, options, type, "trust");
        verifyTrustStoreAlias(fileConfig.alias(), name, type, ks);
    ...
    private static KeyStoreOptions toOptions(KeyStoreFileConfig config,
    ...
    private static KeyStoreOptions toOptions(TrustStoreFileConfig config, TrustStoreCredentialProviderConfig cp, String name,
            String type) {
        KeyStoreOptions options = new KeyStoreOptions();
        try {
            options.setType(type);
